...
namespace Messenger.Core.Services {
  public class ChannelService : AzureServiceBase {
    ...
    /// <returns>The id of the created channel if it was created successfully,
    /// null otherwise</returns>
    public static async Task<uint?> CreateChannel(string channelName,
                                                  uint teamId) {
      LogContext.PushProperty("Method", "CreateChannel");
      LogContext.PushProperty("SourceContext", "ChannelService");

      logger.Information(
          $"Function called with parameters channelName={channelName}, teamId={teamId}");
    ...
    /// affected at least one query, false otherwise</returns>
    public static async Task<bool> RemoveChannel(uint channelId) {
      LogContext.PushProperty("Method", "RemoveChannel");
      LogContext.PushProperty("SourceContext", "ChannelService");

      logger.Information(
          $"Function called with parameters channelId={channelId}");
    ...
    /// </summary>
    /// <param name="teamId">The id of the team to clear the channels
    /// from</param> <returns>True if no exceptions occured while executing the
    /// query and it affected at least one query, false otherwise</returns>
    ...
    /// <returns>True, if the channel got renamed successfully, false
    /// otherwise</returns>
    public static async Task<bool> RenameChannel(string channelName,
                                                 uint channelId) {
      string query = $@"
                                UPDATE
                                    Channels
    ...
    /// <summary>
    /// Construct a Channel object from data that belongs to the channel
    ...
    public static async Task<Channel> GetChannel(uint channelId) {
      ...
      SqlDataAdapter adapter =
          new SqlDataAdapter(selectQuery, GetDefaultConnection());
    ...
    /// <param name="channelId">The id of the channel to pin the message
    /// in</param> <returns>True on success, false on failure</returns>
    public static async Task<bool> PinMessage(uint messageId, uint channelId) {
      LogContext.PushProperty("Method", "PinMessage");
      LogContext.PushProperty("SourceContext", "ChannelService");

      logger.Information(
          $"Function called with parameters messageId={messageId}, channelId={channelId}");
    ...
    /// <param name="channelId">The id of the channel to Unpin the message
    /// in</param> <returns>True on success, false on failure</returns>
    public static async Task<bool> UnPinMessage(uint messageId,
                                                uint channelId) {
    ...
    /// </summary>
    /// <param name="channelId">The channel to retrieve pinned messages
    /// from</param> <returns>An enumerable of message objects representing the
    /// pinned messages</returns>
    public static async Task<IEnumerable<Message>>
    RetrievePinnedMessages(uint channelId) {
