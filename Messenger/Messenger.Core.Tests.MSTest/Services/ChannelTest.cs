  ...
  public class ChannelTest {
    ...
    public void CreateChannel_Test() {
      ...
      Task.Run(async () => {
            ...
            uint? channelId = await ChannelService.CreateChannel(
                testName + "Channel", teamId.Value);
    ...
    public void RemoveChannel_Test() {
      ...
      Task.Run(async () => {
            ...
            var numChannelsBefore =
                (await TeamService.GetAllChannelsByTeamId(teamId.Value))
                    .Count();

            bool success = await ChannelService.RemoveChannel(channelId.Value);

            var numChannelsAfter =
                (await TeamService.GetAllChannelsByTeamId(teamId.Value))
                    .Count();
    ...
    public void RenameChannel_Test() {
      ...
      Task.Run(async () => {
            ...
            bool success = await ChannelService.RenameChannel(
                testName + "ChannelRename", channelId.Value);
    ...
    public void PinMessage_Test() {
      ...
      Task.Run(async () => {
            var _ = await UserService.GetOrCreateApplicationUser(
                new User() { Id = testName + "User" });
            var teamId = await TeamService.CreateTeam(testName + "Team");
            Assert.IsNotNull(teamId);

            var channelId = await ChannelService.CreateChannel(
                testName + "Channel", teamId.Value);
            Assert.IsNotNull(channelId);

            var messageId = await MessageService.CreateMessage(
                channelId.Value, testName + "User", testName + "Text");
            Assert.IsNotNull(messageId);

            var didPinMessage = await ChannelService.PinMessage(
                messageId.Value, channelId.Value);
            Assert.IsTrue(didPinMessage);

            var pinnedMessages =
                await ChannelService.RetrievePinnedMessages(channelId.Value);
            Assert.AreEqual(1, Enumerable.Count(pinnedMessages));

            var pinnedMessageId = pinnedMessages.FirstOrDefault().Id;
            Assert.AreEqual(messageId.Value,
                            pinnedMessages.FirstOrDefault().Id);
          })
          .GetAwaiter()
          .GetResult();
    }
    ...
    public void UnPinMessage_Test() {
      ...
      Task.Run(async () => {
            ...
            var didUnpin = await ChannelService.UnPinMessage(messageId.Value,
                                                             channelId.Value);
