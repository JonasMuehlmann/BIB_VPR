...
namespace Messenger.Tests.MSTest {
  ...
  public class UserServiceTest {
    ...
    public void GetOrCreateApplicationUserExisting_Test() {
      ...
      Task.Run(async () => {
            var data = new User() { Id = testName + "UserId",
                                    DisplayName = testName + "UserName" };

            User retrievedUser =
                await UserService.GetOrCreateApplicationUser(data);
    ...
    public void GetOrCreateApplicationUserSecondNameId_Test() {
      ...
      Task.Run(async () => {
            var user1 = new User() { Id = testName + "UserId1",
                                     DisplayName = testName + "UserName" };
            var user2 = new User() { Id = testName + "UserId2",
                                     DisplayName = testName + "UserName" };

            User retrievedUser =
                await UserService.GetOrCreateApplicationUser(user1);
    ...
    public void UpdateUsername_Test() {
      ...
      Task.Run(async () => {
            var user = new User() { Id = testName + "UserId",
                                    DisplayName = testName + "UserName" };

            User retrievedUser =
                await UserService.GetOrCreateApplicationUser(user);
            Assert.AreEqual(testName + "UserId", retrievedUser.Id);

            bool didRename = await UserService.UpdateUsername(
                testName + "UserId", testName + "UserNameAfter");
    ...
    public void UpdateUserInfo_Test() {
      ...
      Task.Run(async () => {
            ...
            bool didChangeBio = await UserService.Update(
                testName + "UserId", "Bio", testName + "BioAfter");
    ...
    public void SearchUser_Test() {
      ...
      Task.Run(async () => {
            List<User> users = new List<User> {
              new User() { Id = "Id1", NameId = 1,
                           DisplayName = $"{testName}1" },
              new User() { Id = "Id2", NameId = 2,
                           DisplayName = $"{testName}1" },
              new User() { Id = "Id3", NameId = 1,
                           DisplayName = $"The{testName}2" },
              new User() { Id = "Id4", NameId = 1,
                           DisplayName = $"Another{testName}" },
              new User() { Id = "Id5", NameId = 1,
                           DisplayName = $"YetAnother{testName}" },
              new User() { Id = "Id6", NameId = 1,
                           DisplayName = $"ThisIsA{testName}" },
              new User() { Id = "Id7", NameId = 1,
                           DisplayName = $"A{testName}ThisBe" },
              ...
              new User() { Id = "Id11", NameId = 1,
                           DisplayName = $"Deez Nuts {testName}" },
              new User() { Id = "Id12", NameId = 1, DisplayName = $"  " },
              new User() { Id = "Id13", NameId = 1,
                           DisplayName =
                               $"jdhsjdhjdhj{testName}dksdskdjkdjsk" },
              new User() { Id = "Id14", NameId = 1,
                           DisplayName = $"ksjdksjdahdj" },
              new User() { Id = "Id15", NameId = 1,
                           DisplayName =
                               $"jdhsjdhjdhj {testName} dksdskdjkdjsk" }
            };

            var userMatchString =
                $"{testName}1#000001,{testName}1#000002,The{testName}2#000001,Another{testName}#000001,ThisIsA{testName}#000001,A{testName}ThisBe#000001,YetAnother{testName}#000001,Deez Nuts {testName}#000001,jdhsjdhjdhj{testName}dksdskdjkdjsk#000001,jdhsjdhjdhj {testName} dksdskdjkdjsk#000001";
    ...
    public void ChangeBio_Test() {
      ...
      Task.Run(async () => {
            ...
            var user = await UserService.GetOrCreateApplicationUser(
                new User() { Id = userId, DisplayName = testName + "UserName",
                             Bio = testName + "Bio" });
            ...
            var success =
                await UserService.UpdateUserBio(userId, oldBio + "New");
            Assert.IsTrue(success);

            user = await UserService.GetOrCreateApplicationUser(
                new User() { Id = userId });
            Assert.IsNotNull(user);
    ...
    public void ChangeMail_Test() {
      ...
      Task.Run(async () => {
            ...
            var success =
                await UserService.UpdateUserMail(userId, oldEmail + "New");
            Assert.IsTrue(success);
    ...
    public void ChangePhoto_Test() {
      ...
      Task.Run(async () => {
            ...
            var success =
                await UserService.UpdateUserPhoto(userId, oldEmail + "New");
            Assert.IsTrue(success);
    ...
    [TestCleanup]
    public void Cleanup() { ServiceCleanup.Cleanup(); }
  }
}
